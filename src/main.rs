...
use crate::terrain::{HeightMap, TerrainPlugin, TerrainUpdateSet};
...
const SPHERE_RADIUS: f32 = 50_f32;
const CYLINDER_RADIUS: f32 = SPHERE_RADIUS * (3.0 / 16.0);
const CYLINDER_HEIGHT: f32 = SPHERE_RADIUS * 5.0;
...
fn raycast_terrain(
	...
	let step_size = 1.0; // Step size for ray marching
	...
	while t < max_distance {
		...
		if point.y - CYLINDER_HEIGHT * 0.8 <= terrain_height {
...
fn create_pin(
	...
	let sphere_mesh = Sphere::new(SPHERE_RADIUS).mesh().build();
	let cylinder_mesh = Cylinder::new(CYLINDER_RADIUS, CYLINDER_HEIGHT)
		.mesh()
		.build();
	...
	commands
		...
		.with_children(|parent| {
			parent.spawn((
				...
				Transform::from_xyz(0.0, -CYLINDER_HEIGHT * 0.5, 0.0), // Position cylinder below sphere
				Pickable::default(),
			));
		});
}
	...
	if let Some(dragging_entity) = drag_state.dragging_pin {
		if let Ok(mut pin_transform) = pin_query.get_mut(dragging_entity) {
			if let Some(world_pos) = cursor_world_pos.0 {
				...
				pin_transform.translation.y = world_pos.y + CYLINDER_HEIGHT * 0.8;
			}
		}
	}
}
